...
local picker_chrome = require("acp.picker_chrome")
...
test("action picker lines render workflow details", function()
	...
	ok(preview_text:find("<Enter> runs", 1, true))
...
test("floating picker renders preview windows for mapped source rows", function()
	...
	ok(vim.wo[view.preview_winid].winbar:find(icons.note, 1, true))
	ok(vim.wo[view.preview_winid].winbar:find(icons.code, 1, true))
	ok(vim.wo[view.preview_winid].winbar:find("Preview", 1, true))
	ok(vim.wo[view.preview_winid].winbar:find("lua", 1, true))
	ok(vim.wo[view.preview_winid].winbar:find("q close", 1, true))

	view.filter("beta")
	eq(vim.api.nvim_buf_get_lines(view.preview_bufnr, 0, -1, false)[1], "preview row 4")
	ok(vim.wo[view.preview_winid].winbar:find(icons.key, 1, true))
	view.close()
...
test("floating picker starts Tree-sitter for language previews", function()
	...
	local passed, err = pcall(function()
		...
		ok(vim.wo[view.preview_winid].winbar:find(icons.treesitter, 1, true))
		ok(vim.wo[view.preview_winid].winbar:find("Tree-sitter", 1, true))
		...
		ok(vim.wo[view.preview_winid].winbar:find(icons.file, 1, true))
		ok(vim.wo[view.preview_winid].winbar:find("filetype", 1, true))
	end)
...
test("picker chrome renders shared Nerd Font icons", function()
	...
	eq(picker_chrome.footer("Press <Enter> to select."), ("%s Press <Enter> to select."):format(icons.key))
end)
...
test("floating picker applies chrome highlights", function()
	...
	view.filter("missing")
	local missing = table.concat(vim.api.nvim_buf_get_lines(view.bufnr, 0, -1, false), "\n")
	ok(missing:find("0/4 results", 1, true))
	ok(missing:find("No matching picker entries for missing", 1, true))
...
test("prompt view renders ghost text and draft stats", function()
	...
	ok(empty.ghost:find(icons.send, 1, true))
	ok(empty.ghost:find(icons.help, 1, true))
	ok(empty.ghost:find(icons.source, 1, true))
	...
	ok(ribbon_text:find(("diagnostics %s1 %s1"):format(icons.error, icons.warning), 1, true))
...
test("session panel view renders status and badges", function()
	...
	ok(text:find(icons.session, 1, true))
	ok(text:find(("%s #1 test test-model"):format(icons.location), 1, true))
	ok(text:find(icons.session, 1, true))
	ok(text:find("streaming", 1, true))
	ok(text:find("2 change(s)", 1, true))
	ok(text:find(icons.status, 1, true))
	ok(text:find(icons.changes, 1, true))
	ok(text:find("error: failed", 1, true))
	eq(line_ids[3], 1)
	eq(line_ids[4], 1)
	eq(styles[1].line_hl_group, "AcpSessionHeader")
	local header_hint = styles[1].virt_text[1][1]
	ok(header_hint:find(icons.enter, 1, true))
	ok(header_hint:find("<Enter> focus", 1, true))
	ok(header_hint:find(icons.close, 1, true))
	ok(header_hint:find("x close", 1, true))
	ok(header_hint:find(icons.action, 1, true))
	ok(header_hint:find("<leader>ak actions", 1, true))
...
test("session panel view renders transcript and source details", function()
	...
	ok(text:find("3 sec", 1, true))
	ok(text:find("2 code", 1, true))
	ok(text:find("1 loc", 1, true))
	ok(text:find(icons.section, 1, true))
	ok(text:find(icons.code, 1, true))
...
test("restore session view renders metadata and preview", function()
	...
	ok(text:find(icons.session, 1, true))
	ok(text:find(icons.history, 1, true))
	...
	ok(preview_text:find(icons.note, 1, true))
	ok(preview_text:find(icons.session, 1, true))
	ok(preview_text:find(icons.history, 1, true))
	ok(preview_text:find(icons.location, 1, true))
	ok(preview_text:find(icons.model, 1, true))
...
test("source view renders LSP document highlights", function()
	...
	eq(link_badge_mark.opts.sign_text, icons.link)
...
test("output dashboard and section helpers are rendered", function()
	...
	ok(timeline[3].label:find(icons.user, 1, true))
	...
	ok(outline_text:find(icons.user, 1, true))
	...
	ok(transcript_text:find(icons.user, 1, true))
	ok(transcript_text:find(icons.note, 1, true))
	...
	ok(fold_text:find(icons.user, 1, true))
	ok(fold_text:find("USER", 1, true))
	ok(fold_text:find("2 lines", 1, true))
	ok(fold_text:find("hello", 1, true))

	local motion_frame = icons.location .. string.rep(icons.pulse_empty, 4)
	eq(acp_output.animation_frame(1), icons.pulse_empty)
	eq(acp_output.animation_frame(5), icons.pulse_empty)
	eq(acp_output.motion_frame(1), motion_frame)
	eq(acp_output.motion_frame(9), motion_frame)
	...
	ok(skyline:find(icons.pulse_empty, 1, true))
	ok(skyline:find(icons.section, 1, true))
	ok(skyline:find(icons.code, 1, true))
	ok(skyline:find(icons.location, 1, true))
	...
	ok(activity_badge:find(icons.pulse_mid .. " streaming", 1, true))
	...
	ok(activity_badge:find(acp_output.motion_frame(2), 1, true))
	...
	ok(tool_lens[4][1]:find(icons.inspect, 1, true))
	...
	ok(live_status:find(icons.pulse_mid .. " " .. icons.status .. " live: streaming", 1, true))
	ok(live_status:find(acp_output.motion_frame(2), 1, true))
	...
	ok(block_header:find(("lua %s lua"):format(icons.arrow_right), 1, true))
	...
	ok(block_text:find("Q quickfix", 1, true))
	...
	ok(ref_text:find("Q quickfix", 1, true))
	...
	local half_progress = string.rep(icons.pulse_full, 5) .. string.rep(icons.pulse_empty, 5)
	eq(acp_output.progress_bar(3, 6, 10), half_progress)
	ok(map_text:find("PROBLEM", 1, true))
	ok(map_text:find("CODE", 1, true))
	ok(map_text:find("REFERENCE", 1, true))
	ok(map_text:find(("%s %s"):format(icons.location, half_progress), 1, true))
	ok(map_text:find("<Enter> jump", 1, true))
	ok(map_text:find("K preview", 1, true))
	ok(map_text:find("Q quickfix", 1, true))
...
test("diagnostic picker lines render source and code", function()
	...
	ok(text:find(icons.diagnostics, 1, true))
	ok(text:find(icons.note, 1, true))
...
test("LSP inlay hints are flattened for picker rows", function()
	...
	ok(text:find(icons.hint, 1, true))
...
test("LSP document colors are normalized and rendered", function()
	...
	ok(text:find(icons.color, 1, true))
...
test("config option picker lines render selectable options", function()
	...
	ok(text:find(icons.config, 1, true))
	ok(text:find("Session Mode: Ask [mode]", 1, true))
	ok(text:find(icons.inspect, 1, true))
	ok(text:find(icons.type, 1, true))
	ok(text:find("select", 1, true))
	ok(text:find("2 values", 1, true))
	ok(text:find(icons.package, 1, true))
	...
	ok(value_text:find("1.", 1, true))
	ok(value_text:find("Ask", 1, true))
	ok(value_text:find(icons.preferred, 1, true))
	ok(value_text:find("current", 1, true))
	ok(value_text:find("Request permission", 1, true))
	ok(value_text:find("2.", 1, true))
	ok(value_text:find("Code", 1, true))
	eq(line_values[3].value, "ask")
	eq(line_values[5].value, "code")

	local preview = acp_config.preview(line_options[3])
	local preview_text = table.concat(preview.lines, "\n")
	eq(preview.filetype, "acp-sessions")
	ok(preview.title:find("ACP config Session Mode", 1, true))
	ok(preview_text:find("Current: Ask", 1, true))
	ok(preview_text:find("Id: mode", 1, true))
	ok(preview_text:find("Type: select", 1, true))
	ok(preview_text:find("Category: mode", 1, true))
	ok(preview_text:find("Values", 1, true))
	ok(preview_text:find("Ask current", 1, true))

	local value_preview = acp_config.value_preview(line_options[3], line_values[5])
	local value_preview_text = table.concat(value_preview.lines, "\n")
	eq(value_preview.filetype, "acp-sessions")
	ok(value_preview.title:find("ACP value Code", 1, true))
	ok(value_preview_text:find("Option: Session Mode", 1, true))
	ok(value_preview_text:find("Value: code", 1, true))
	ok(value_preview_text:find("State: available", 1, true))
end)
...
test("slash command picker lines and draft text are rendered", function()
	...
	ok(text:find(icons.command, 1, true))
	ok(text:find("/plan", 1, true))
	ok(text:find("Create a plan", 1, true))
	ok(text:find("input: task", 1, true))
	ok(text:find("/test", 1, true))
	ok(text:find("no input", 1, true))
	eq(acp_commands.slash_text(line_commands[3]), "/plan ")
	eq(acp_commands.slash_text(line_commands[6]), "/test")

	local preview = acp_commands.preview(line_commands[3])
	local preview_text = table.concat(preview.lines, "\n")
	eq(preview.filetype, "acp-sessions")
	ok(preview.title:find("ACP command /plan", 1, true))
	ok(preview_text:find("Draft: /plan ", 1, true))
	ok(preview_text:find("Input: task", 1, true))
	ok(preview_text:find("Create a plan", 1, true))

	local no_input_preview = acp_commands.preview(line_commands[6])
	local no_input_text = table.concat(no_input_preview.lines, "\n")
...
test("prompt completion backend items are rendered for blink", function()
	...
	eq(plan_items[1].abbr, ("%s /plan"):format(icons.command))
	eq(plan_items[1].icon, icons.command)
	eq(plan_items[1].menu, ("%s task"):format(icons.key))
	eq(plan_items[1].info, "Create a plan")

	local all_items = acp_commands.completion_items(commands, "/")
	eq(#all_items, 2)
	eq(all_items[2].word, "/test")
	eq(all_items[2].menu, icons.title("ACP"))
	...
	eq(workflow_items[1].menu, ("%s source"):format(icons.source))
	...
	eq(code_action_item.menu, ("%s LSP"):format(icons.code))
...
test("prompt history recalls sent prompts and restores draft", function()
	...
	local passed, err = pcall(function()
		...
		for _, mark in ipairs(marks) do
			...
			for _, line in ipairs((mark[4] and mark[4].virt_lines) or {}) do
				...
				if
					...
					and text:find(("diagnostics %s1"):format(icons.error), 1, true)
		...
		eq(blink_result.items[1].labelDetails.description, ("%s source"):format(icons.source))
		ok(blink_result.items[1].documentation.value:find(("context: %s source"):format(icons.source), 1, true))
		ok(blink_result.items[1].documentation.value:find(icons.source, 1, true))
		ok(blink_result.items[1].documentation.value:find("insert: @context", 1, true))
		ok(blink_result.items[1].documentation.value:find("action: runs ACP workflow", 1, true))
...
test("output buffer shows dashboard, chrome, and section navigation", function()
	...
	local passed, err = pcall(function()
		...
		ok(vim.wo[problem_preview_win].winbar:find(icons.file, 1, true))
		...
		ok(loclist.title:find(icons.error, 1, true))
		eq(#loclist.items, 1)
		...
		for _, mark in ipairs(marks) do
			...
			for _, chunk in ipairs((mark[4] and mark[4].virt_text) or {}) do
				if chunk[1] and chunk[1]:find(("lua %s lua"):format(icons.arrow_right), 1, true) then
					code_badge = true
				end
				if mark[2] == code_line - 1 and chunk[1] and chunk[1]:find(acp_output.motion_frame(1), 1, true) then
			...
			for _, virt_line in ipairs((mark[4] and mark[4].virt_lines) or {}) do
				...
				if
					...
					and rendered:find(("lua %s lua"):format(icons.arrow_right), 1, true)
		...
		ok(vim.wo[0].winbar:find(icons.code, 1, true))
		ok(vim.wo[0].winbar:find(icons.section, 1, true))
		ok(vim.wo[0].winbar:find("<leader>at scope", 1, true))
		ok(vim.wo[0].winbar:find(icons.scope, 1, true))
		ok(vim.wo[0].winbar:find("<leader>ai draft", 1, true))
		ok(vim.wo[0].winbar:find(icons.prompt, 1, true))
		ok(vim.wo[0].winbar:find("gO output", 1, true))
		ok(vim.wo[0].winbar:find(icons.map, 1, true))
		ok(vim.wo[0].winbar:find("<leader>aY yank", 1, true))
		ok(vim.wo[0].winbar:find(icons.reference, 1, true))
		ok(
		...
		ok(code_qflist.title:find(icons.quickfix, 1, true))
		...
		ok(output_map_text:find(icons.pulse_full, 1, true))
		...
		ok(vim.wo[ref_preview_win].winbar:find(icons.note, 1, true))
...
test("changes module records unique written files for picker and quickfix", function()
	...
	local qf = vim.fn.getqflist({ title = 1, items = 1 })
	eq(qf.title, icons.title("ACP changes #12", icons.quickfix))
	eq(#qf.items, 2)
	vim.cmd("cclose")
...
test("file write review renders a diff preview", function()
	...
	ok(text:find(icons.file, 1, true))
	ok(text:find(icons.edit, 1, true))
...
test("permission UI formats tool details and options", function()
	...
	ok(text:find(icons.warning, 1, true))
	ok(text:find(icons.action, 1, true))
