...
function M.picker_lines(items)
	local lines = { chrome.title(icons.link, "ACP Document Links"), "" }
	local line_items = {}
	for index, item in ipairs(items or {}) do
		local range = M.range(item) or {}
		table.insert(
			lines,
			chrome.row(index, icons.link, ("%d:%d  %s"):format(range.line1 or 1, range.col1 or 1, M.label(item)))
		)
		line_items[#lines] = item
		if M.tooltip(item) and M.tooltip(item) ~= M.label(item) then
			table.insert(lines, chrome.detail(icons.note, short(M.tooltip(item), 96)))
			line_items[#lines] = item
		end
	end

	table.insert(lines, "")
	table.insert(lines, chrome.footer("Press <Enter> to add link context, Q for quickfix, or q/<Esc> to close."))
