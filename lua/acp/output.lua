...
local function section_label(line)
	...
	if heading then
		return heading:upper(), heading
	end
	local quote = line:match("^>%s*(.*)")
	if quote then
		if quote:match("^Error:") or quote:match("failed") then
			return "ERROR", quote:gsub("^Error:%s*", "")
		elseif quote:match("^Warning:") then
			return "WARNING", quote:gsub("^Warning:%s*", "")
		elseif quote:match("^Command") then
			return "COMMAND", quote
		elseif quote:match("^Tool") then
			return "TOOL", quote
		elseif quote:match("^Context:") then
			return "CONTEXT", quote:gsub("^Context:%s*", "")
		elseif quote:match("^File changes") or quote:match("^[%a%s]+%s+`[^`]+`") then
			return "FILE", quote
		end
		return "NOTE", quote
	end
...
local function section_icon(kind)
	...
	if kind == "PLAN" or kind == "REVIEW" then
